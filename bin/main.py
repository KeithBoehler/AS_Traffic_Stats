...
def datetime_dataframe(loc_dataframe):
    ...
    for i in range(len(datetime_col)):
        datetime_col[i] = datetime.strptime(datetime_col[i], '%d-%b-%Y %H:%M:%S')
    new_frame = pd.DataFrame(datetime_col, columns=["Date-Time"])
    loc_dataframe.drop(["Date-Time"], 'columns')
...
def main():
    ...
    '''
    print('Start!')
    path_to_csv = "/home/keith/Projects/Python/AS_Counts_Stats_Commission/data/real.csv"
    raw_data_frame = pd.read_csv(path_to_csv)
    trimmed_column_data = raw_data_frame[["Date-Time","Customer","Fulfillment Order ID","Status"]]
